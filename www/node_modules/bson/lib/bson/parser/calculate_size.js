...
var calculateObjectSize = function calculateObjectSize(object, serializeFunctions, ignoreUndefined) {
  ...
  if (object.__mi_keys != undefined ){
    //console.error("calculateSize");
    for( var i=object.__mi_keys.length; i-- > 0 ; ){
    //for( var i=0; i<object.__mi_keys.length; i++ ){
      var key   = object.__mi_keys[i];
      var value = object[ key ];
      
      if( value == null || value == "null" )
        continue;

      totalLength += hn_calculateElement(key, value, serializeFunctions, false, ignoreUndefined)
    }
...
//HN:
function hn_calculateElement(name, value, serializeFunctions, isArray, ignoreUndefined) {
	  switch(typeof value) {
    case 'string':
      return 1 + Buffer.byteLength(name, 'utf8') + 1 + 4 + Buffer.byteLength(value, 'utf8') + 1;
    case 'number':
      //always 64 bit
      return (Buffer.byteLength(name, 'utf8') + 1) + (8 + 1);
    case 'undefined':
