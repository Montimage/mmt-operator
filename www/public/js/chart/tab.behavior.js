...
/**
 * split db.data into two parts having timestamp (1) same as the db.time.end and
 * (2) the rest
 */
...
function myGraph(domID, root) {
   ...
   this.addIp = function (ip, cat_name) {
      for (var i = 0; i < root.children.length; i++) {
         ...
         // flash
         /*
          * setTimeout(function () { $('g[name="' + ip + '"]').animate({
          * opacity: 0 }, 500, "linear", function () { $(this).animate({
          * opacity: 1 }, 300, "linear", function () { $(this).animate({
          * opacity: 0 }, 300, "linear", function () { $(this).animate({
          * opacity: 1 }, 300); }); }); }); }, 1000)
          */
         return;
      }
