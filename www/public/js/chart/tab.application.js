...
var ReportFactory = {
    ...
    createResponseTimeReport: function (fPeriod) {
        ...
        [COL.DATA_VOLUME.id].forEach( function( el, index){
          group[ el ] = {"$sum" : "$" + el};
        });
        [COL.APP_ID.id ].forEach( function( el, index){
          group[ el ] = {"$first" : "$"+ el};
        ...
        var appList_db = new MMTDrop.databaseFactory.createStatDB(
        ...
        var database = new MMTDrop.Database({collection: "data_app", action: "aggregate"},
          function( data ){
            ...
            var divide = function( a, b ){
              if (b == 0) return 0;
              return Math.round( a * 100/b )/100; //equivalent with toFixed(2) but return a number (instanceof string)
            }
            //format data
            for( var i=0; i<data.length; i++ ){
              ...
              data[i][ COL.RTT.id ]            = divide( data[i][ COL.RTT.id ], val);

              val = data[i][ HTTP.TRANSACTIONS_COUNT.id ] * 1000; //micro second => milli second

              data[i][ HTTP.RESPONSE_TIME.id ] = divide( data[i][ HTTP.RESPONSE_TIME.id ]     , val);
              data[i][ HTTP.DATA_TRANSFER_TIME.id ] = divide( data[i][ HTTP.DATA_TRANSFER_TIME.id ], val);

              //% of retransmision
              data[i][ COL.RETRANSMISSION_COUNT.id ] = divide( data[i][ COL.RETRANSMISSION_COUNT.id ]*100, data[i][ COL.PACKET_COUNT.id ] );
              //% of payload
              data[i][ COL.PAYLOAD_VOLUME.id ] = divide( data[i][ COL.PAYLOAD_VOLUME.id ]*100, data[i][ COL.DATA_VOLUME.id ] );
              //bit per second
              data[i][ COL.DATA_VOLUME.id ]    = divide( data[i][ COL.DATA_VOLUME.id ]*8, time_interval );
              //pps
              data[i][ COL.PACKET_COUNT.id ]   = divide( data[i][ COL.PACKET_COUNT.id ], time_interval );
            }
                        ...
                        chart   : {
                          grid: {
                            y: {
                              lines : [
                                //granularity/average
                                {value: total, text: total + " ms", position: 'start'}
                              ]
                            }
                          }
                        }
            ...
            getData: {
                getDataFn: function (db) {
                    ...
                    for( var i=0; i<data.length; i++ ){
                        ...
                        var t2                  = _this.formatTime( new Date( time + fPeriod.getDistanceBetweenToSamples() * 1000 ) );

                        //user can see in detail if this row has data
                        if( msg[ COL.DATA_VOLUME.id ] > 0 ){
                            var load_fn = 'loadDetail('+ time +','+ app_id +')';
                            msg[ COL.TIMESTAMP.id ] = '<a data-timestamp='+ time +' onclick='+ load_fn +'>'
        ...
        fApp.onFilter( function( opt ){
            ...
            var app_id  = opt.id;
            if( app_id != 0 )
              $match[ COL.APP_ID.id   ] = parseInt( app_id );

            var db_options = {query : [{"$match": $match},{"$group" : group}],
    ...
    createSlowestAppsReport: function (fPeriod ){
      return this.createSlowestUsersReport( fPeriod, true );
    },
    createSlowestUsersReport: function (fPeriod, is_app) {
        var _this = this;
        var COL   = MMTDrop.constants.StatsColumn;
        var HTTP  = MMTDrop.constants.HttpStatsColumn;
        var FTP   = MMTDrop.constants.FtpStatsColumn;

        is_app    = (is_app !== undefined);
        var col_id = COL.IP_SRC.id;
        if( is_app )
        ...
        return MMTDrop.chartFactory.createTable({
            getData: {
                getDataFn: function (db) {
                    ...
                    var col_key  = {id: COL.IP_DEST.id,  label: "Remote Addr" };
                    ...
                    for( var i=0; i<arr.length; i++ ){
                        ...
                        obj[ HTTP.DATA_TRANSFER_TIME.id ] = self.formatRTT( obj[ HTTP.DATA_TRANSFER_TIME.id ] );
