...
*/
	...
	autoPerformReaction: function(alert){
		...
		if(! config.modules_config.l4s.auto_perform_reaction)
			return false;
		const now = alert[COL.TIMESTAMP];
		
		//at most one alert per minute
		if( now - last_execution_script_ts < 60*1000 ){
			console.log("Ignored execution of reaction script as it was executed recently less than 1 minute");
			return;
		}
		...
		const script_name = path.join(cwd, "l4s-block-traffic.sh");
		const ret = exec.exec( script_name, ["ips", findConcernedIPs(alert[8])], {cwd: cwd} );
