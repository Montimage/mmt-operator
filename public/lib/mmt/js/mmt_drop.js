...
MMTDrop.constants = {
		...
		getProtocolNameFromID : function(id) {
			var protocolName;
			protocolName = ( id in MMTDrop.constants.ProtocolsIDName) ? MMTDrop.constants.ProtocolsIDName[id] : 'n/a';
			return protocolName;
		},
...
MMTDrop.chartFactory = {
		...
		createTree : function ( param ){
			...
			var chart = new MMTDrop.Chart( _param, 
				...
				//get protocol parent of `path`
				//for example: parent of `HTTP` is `Ethernet.IP.IP.TCP`
				// the paren could be also `Ethernet.IP` if there are no packet `Ethernet.IP.IP.TCP` nor `Ethernet.IP.IP` in `data` 
				var _getParent = function( path ){
					var name = path;
					var parent = null;
				...
				for (var i in arrData){
					...
					//separate APP_PATH
					var path = arrData[i][0];
					var obj = _getParent( path );
					var name   = obj.name;
					var parent = obj.parent;
					//translate name from number code to string
